...
function util.write_row(type, uid, delta, source, attacker, damage, crit, apply, element, reaction, amp_type, amp_rate, count, aid, mid, defender)
	write_col(type, 6)
	write_col(uid, 6)
	write_col(delta, 7)
	write_col(source, 35)
	...
	write_col(amp_rate, 8)
	write_col(count, 5)
	write_col(aid, 3)
	write_col(mid, 3)
	write_col(defender, 35, nil, true)
	util.reset_style()
	util.write("\n")
	odd_row = not odd_row
end
